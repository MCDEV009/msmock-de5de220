...
ON public.subjects FOR SELECT TO authenticated, anon
USING (true);
...
ON public.tests FOR SELECT TO authenticated, anon
USING (visibility = 'public' OR public.has_role(auth.uid(), 'admin'));
...
ON public.questions FOR SELECT TO authenticated, anon
USING (
    EXISTS (
...
-- Create test_participants table (for anonymous users taking tests)
...
ON public.test_participants FOR INSERT TO authenticated, anon
WITH CHECK (true);

CREATE POLICY "Participants viewable by admins"
...
ON public.test_attempts FOR INSERT TO authenticated, anon
WITH CHECK (true);

CREATE POLICY "Anyone can update their attempt"
ON public.test_attempts FOR UPDATE TO authenticated, anon
USING (true);

CREATE POLICY "Attempts viewable by participant or admin"
ON public.test_attempts FOR SELECT TO authenticated, anon
USING (true);

-- Function to generate unique participant ID
...
-- Function to generate 5-digit test code
...
-- Trigger to auto-generate test code for private tests
...
CREATE TRIGGER trigger_set_test_code
BEFORE INSERT ON public.tests
FOR EACH ROW
EXECUTE FUNCTION public.set_test_code();
...
('Ona tili', 'Родной язык', 'Native Language'),
